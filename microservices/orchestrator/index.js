  ...
  try {
    ...
    const productDetailsPromises = orderProducts.map(async (orderProduct) => {
      const productResponse = await axios.get(
        `http://product-backend:8080/products/${orderProduct.product_id}`
      );
    ...
    const userInfoResponse = await axios.get(
      `http://user-backend:8081/users/${customerId}`
    );
    const userInfo = userInfoResponse.data;

    const userProfileInfoResponse = await axios.get(
      `http://user-backend:8081/profile/${customerId}`
    );
    ...
    res
      .status(500)
      .json({ message: "Error al obtener el recibo", error: error.message });
  }
});
...
app.get("/orchestrator/users/:userId/favorites", async (req, res) => {
  ...
  try {
    const favProductsResponse = await axios.get(
      `http://user-backend:8081/favProducts/${userId}`
    );
    ...
    if (!favProducts || favProducts.length === 0) {
      return res
        .status(404)
        .json({ message: "No favorite products found for this user" });
    }

    const productDetailsPromises = favProducts.map(async (favProduct) => {
      const productResponse = await axios.get(
        `http://product-backend:8080/products/${favProduct.product_id}`
      );
      const product = productResponse.data;
      return {
        product_id: product.id,
